...
** Exercise 1.25
  ...
  Alyssa is correct. =expmod= works by generating the next exponent for the function every call, effectively reaching the exponent of a^n (starting from a^0). However, this method is incredibly convoluted and can be further enhanced by using the =fast-expt= function, which first calculates the value of a^n and then calculates the remainder of the exponent.
